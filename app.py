...
<div id="attack" class="page">
    ...
    <!-- Live Attack Feed -->
    <div class="box">
        <div class="box-head"><span>📡 LIVE BATTLEFIELD</span></div>
        <div id="attackChat" class="scroll-list" style="height:200px;"></div>
    </div>
    
    <div class="box">
        <div class="box-head"><span>🔫 ACTIVE SHOOTERS</span></div>
    ...
    class Bot {
        ...
        connect() {
            ...
            this.ws.onmessage = (e) => {
                ...
                // --- THE CRITICAL FIX: "you_joined" ---
                // Server sends "you_joined" when current bot enters
                else if(data.handler === "room_event" && data.type === "you_joined") {
                    this.joined = true;
                    renderBots();
                    log(`${this.user} Joined!`);
                    
                    // Capture User List immediately from "you_joined"
                    if(this.isMaster && data.users && Array.isArray(data.users)) {
                        data.users.forEach(u => usersMap.set(u.username, u));
                        renderUsers();
                    }
                }
                
                // Fallback for "room_joined"
                ...
                if(this.isMaster) {
                    ...
                    if(data.handler === "room_event" && data.type === "text") {
                        addChatMessage(data.from, data.body, myBotNames.includes(data.from));
                    }
                }
            };
